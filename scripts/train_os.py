...
tok = Tokenizer.from_file('precomputed/tokenizer.json')
...
class TrainingArguments:
    ...
    warmup_steps: int = field(default=3, metadata={"help": "Linear warmup over warmup_steps."})
    logging_steps: int = field(default=100, metadata={"help": "Log every X updates steps."})
    save_steps: int = field(default=300, metadata={"help": "Save checkpoint every X updates steps."})
    eval_steps: int = field(default=300, metadata={"help": "Run an evaluation every X steps."})
...
class ModelArguments:
    ...
    config = transformers.BertConfig(
        ...
        pad_token_id=tok.get_vocab_size()
    )
    tokenizer = tok
...
def main():
    ...
    if isinstance(features[label_column_name].feature, ClassLabel):
        ...
        label_list = list(map(str, range(-5, 9)))
    ...
    model = transformers.FlaxBertForTokenClassification(config=config)
    ...
    # p_train_step = jax.vmap(train_step, axis_name='batch')
